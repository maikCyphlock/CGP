...
// Tipo de señalado (valor del <select data-campo="tipo-senalado">) -> código respondent_type de la BD
var MAPA_TIPO_SENALADO = {
...
// value del <select id="cit-tipo-doc"> -> código id_document_type de la BD
var MAPA_TIPO_DOC = { V: 'V', E: 'E', P: 'PASSPORT' };

/** Recolecta las tarjetas de #lista-senalados como { typeCode, location, attributes }. */
function recolectarSenalados() {
    var tarjetas = document.querySelectorAll('#lista-senalados .senalado-card');
    var resultado = [];
    tarjetas.forEach(function (tarjeta) {
        var selectTipo = tarjeta.querySelector('[data-campo="tipo-senalado"]');
        var tipoValor = selectTipo ? selectTipo.value : '';
        var typeCode = MAPA_TIPO_SENALADO[tipoValor];
        if (!typeCode) return;

        var attributes = {};
        var location = '';
        tarjeta.querySelectorAll('.senalado-card-campos [data-campo]').forEach(function (campo) {
            var valor = (campo.value || '').trim();
            if (!valor) return;
            attributes[campo.dataset.campo] = valor;
            if (/direccion|circuito|parroquia$/.test(campo.dataset.campo) && !location) {
                location = valor;
            }
        });
        ...
        resultado.push({
            typeCode: typeCode,
            location: location || 'No especificada en el formulario',
...
/**
 * Valida la declaración jurada y envía la solicitud al backend (Express + BD),
 * que crea el ciudadano (si es nuevo), el expediente (case_file) y los
 * señalados asociados. Muestra la confirmación con el número real generado
 * por el servidor (annual_sequence), no uno aleatorio del cliente.
 */
async function enviarSolicitud() {
    ...
    var tipoTramiteEl = document.querySelector('input[name="tipo_tramite"]:checked');
    var MAPA_TRAMITE = { denuncia: 'COMPLAINT', queja: 'GRIEVANCE', reclamo: 'CLAIM', peticion: 'PETITION' };

    var payload = {
        idDocType: MAPA_TIPO_DOC[document.getElementById('cit-tipo-doc').value] || 'V',
        idDocNumber: document.getElementById('cit-nro-doc').value,
        firstName: (document.getElementById('cit-primer-nombre').value + ' ' + (document.getElementById('cit-segundo-nombre').value || '')).trim(),
        lastName: (document.getElementById('cit-primer-apellido').value + ' ' + (document.getElementById('cit-segundo-apellido').value || '')).trim(),
        sex: document.getElementById('cit-sexo').value || 'M',
        birthDate: document.getElementById('cit-fecha-nac').value || null,
        email: document.getElementById('cit-correo').value,
        mobilePhone: (document.getElementById('cit-telf-cel-cod').value || '') + (document.getElementById('cit-telf-cel-num').value || ''),
        address: document.getElementById('cit-direccion').value,
        parish: document.getElementById('cit-parroquia').value || null,
        municipality: document.getElementById('cit-municipio').value || 'Páez',
        city: document.getElementById('cit-ciudad').value || null,
        claimTypeCode: tipoTramiteEl ? (MAPA_TRAMITE[tipoTramiteEl.value] || 'COMPLAINT') : 'COMPLAINT',
        narrative: document.getElementById('narracion').value,
        incidentDate: new Date().toISOString().slice(0, 10),
        otherInstance: !!(document.querySelector('input[name="otra_instancia"]:checked') && document.querySelector('input[name="otra_instancia"]:checked').value === 'si'),
        otherInstanceName: (document.getElementById('cual-instancia') || {}).value || null,
        isPopularConsultation: !!(document.querySelector('input[name="es_consulta"]:checked') && document.querySelector('input[name="es_consulta"]:checked').value === 'si'),
        popularConsultation: {
            projectName: (document.getElementById('proy-nombre') || {}).value,
            approvalDate: (document.getElementById('proy-fecha') || {}).value,
            projectAmount: Number((document.getElementById('proy-monto') || {}).value || 0),
            fundingEntity: (document.getElementById('proy-financiador') || {}).value
        },
        respondents: recolectarSenalados()
    };
    ...
    var nroExpediente;
    try {
        var respuesta = await fetch('/denuncias', {
            method: 'POST',
        ...
        nroExpediente = datos.caseNumber;
    } catch (e) {
        if (btnEnviar) btnEnviar.disabled = false;
        mostrarBannerError(5, [e.message || 'Error de conexión con el servidor. Intente nuevamente.']);
        return;
    }
